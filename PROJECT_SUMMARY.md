...
**布局重构（v1.4.0 初始）：**
- **侧栏布局修复**：弃用 PanedWindow（macOS 下 weight=0 无效导致侧栏被拉伸），改用 pack 固定宽度布局
- **每日推送移至右侧内容区**：从侧栏底部移至「检索参数」和「邮箱配置」之间的独立卡片
- **每日推送信息卡**：显示运行状态、监控期刊数、推送时间（每日 08:00）、累计推送论文数
- **侧栏底部任务计数**：显示「N 个任务」
- **工具栏精简**：右侧按钮从文字改为图标+文字（⬇更新 / ?说明 / ✉反馈）
- **品牌名精简**：从 "HONGXUN · 论文发现工具" 改为 "HONGXUN"
- **Notebook Tab VS Code 风格**：选中态仅文字变色，无背景色变化
- **侧栏与内容区分隔线**：移除（底色差异已足够）
- **卡片边框跨平台修复**：从 highlightthickness 改为 relief="solid" + borderwidth=1
- **状态栏代码锁移除**：代码锁按钮已从状态栏删除
- **右侧推送卡片 bg 统一**：所有卡片使用 bg_card

**功能改进（v1.4.0 初始）：**
- **一键检索**：自动保存到 output/ 目录，不强制切换 Tab
- **示例填充**：「🌰 试试示例」一键填入 Nature/Science/Cell 预置数据
- **macOS 原生通知**：检索完成使用系统通知
- **首次运行向导**：检测无任务时弹出 3 步引导
- **Logger 系统**：28 处 except:pass → logger.warning(exc_info=True)
- **硬编码 SMTP 凭据集中管理**
- **任务卡「⋯」更多菜单**：执行检索 / 复制任务 / 禁用 / 删除
- **删除按钮**：替换原「重置」按钮为「🗑 删除」
- **运行指示器**：仅检索执行时任务卡片显示蓝色 ◉ 运行状态

**自动更新修复（v1.4.0 初始）：**
- **版本号同步**：`auto_updater.py` 的 CURRENT_VERSION 从 `theme.py:APP_VERSION` 注入，消除版本号漂移
- **每版本仅推送一次**：通知标记字段 `skip_version` → `notified_versions` 列表
- **语义化版本比较**：新增 `_parse_version()` 元组比较，修复字符串比较 bug
- **更新弹窗升级**：自定义对话框展示可滚动版本功能介绍，提供「立即更新」「稍后提醒」「关闭（下次继续提醒）」三种操作
- **手动检查**：工具栏 ⬇ 按钮不受通知标记影响，始终可查

**检索与推送改进（v1.4.0 后期）：**
- **检索保存路径**：用户通过文件对话框选择保存 .doc 报告的路径，不再强制存入 output/
- **输入分隔符灵活性**：期刊和关键词输入支持英文分号、中文分号、英文逗号、中文逗号、顿号混用
- **每日推送改用 from-index-date**：解决 CrossRef 日级别过滤问题，确保 24h 增量窗口的论文准确获取
- **调度守护进程路径修复**：`scheduler_daemon.py` BASE_DIR 多退一层目录导致 PID 文件写入错误路径，GUI 超时

**并发支持——每日推送与执行检索可同时运行：**
- **双 Session 池**（`session.py`）：每日推送使用独立 `_session_bulk` 连接池，不争抢历史检索的连接
- **摘要补全串行锁**（`abstract.py`）：`_enrich_lock` 确保同一时刻只有一个调用方执行 HTTP 摘要补全，避免 API 限速
- **取消标志解耦**（`engine.py`）：单 `threading.Event` → `dict[str, Event]` token 注册系统，各自独立取消
- **进度条主人制**（`gui_app.py`）：`_progress_owner` 缓存非主人方的进度更新，操作结束时自动转交展示

**文献书架性能优化：**
- **弃用 PaperCard 控件**：每张卡 5+ tkinter 控件 → 100 张卡 500+ 控件，导致切 Tab 数秒卡顿
- **改用 ttk.Treeview**：原生支持上千行无额外控件开销，书架 Tab 切换瞬时完成
- **自动标记已读**：单击选中自动设状态为已读，方向键切换也自动已读
- **状态图标**：`○` 灰色待读 / `▲` 绿色已读 / `★` 红色排除，形状+颜色双重区分
- **双击检测**：200ms 定时器判定双击/单击，状态列点击不参与双击检测
- **双击弹窗**：弹出自包含的论文详情窗口（标题/作者/期刊/DOI/可滚动摘要）
- **右栏可滚动**：MetadataCard 支持 scrollable，超出高度时滑轨+滚轮滑动
- **DOI 打开浏览器**：点击 DOI 自动打开 `https://doi.org/{doi}`
